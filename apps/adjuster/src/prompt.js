...
function buildPrompt(input) {
  ...
  var system = [
    ...
    'Two shapes of extracted text. A clause field (its field-specific guidance says so) is a fragment that completes a fixed template sentence: a noun phrase, no leading capital, no finite verb of its own, no trailing period. A narrative field is prose you compose: complete, grammatical sentences with a subject and a finite verb, in report voice. Getting these backwards is a defect either way — a clause that reads as a full sentence, or a narrative that reads as a clause or a noun pile.',
    'Narrative fields must read as prose a person wrote, not dictation copied verbatim: complete sentences ending in terminal punctuation, no dictation artifacts ("okay so", "uh", "let\'s see"), no telegraphic noun piles (write "eight wind-damaged shingles", not "eight wind damage shingles"), numbers and measurements in digits matching the digits already used elsewhere in the same sentence, and one consistent register across parallel fields — the four slopes and the four elevations should not disagree about "damage" versus "damages". A narrative field does not open with a stock no-damage sentence and then go on to describe damage; when there are findings, write the findings, and save the no-damage sentence for a slope or elevation with none. When a field\'s guidance names content it must include (e.g. a determination plus a reason), the field is not complete without that content.',
    "A narrative's value is composed by you, not copied: its source_span is the transcript passage the composition is grounded in, still an exact verbatim substring per the source_span rule below, but the value itself should read as a written sentence rather than an echo of the span. A narrative that repeats its source_span nearly word for word is usually a sign it was copied rather than composed, and should be rewritten as a proper sentence covering the same facts.",
    ...
    'Status variants (roof, exterior, interior, personal property, mitigation, mortgage, coinsurance) require an affirmative statement — "the roof was not affected", "there is no mortgage", "no coinsurance penalty applies" — before you choose a value. A direct answer to a direct question is an affirmative statement even when it is one word: where the agent asks "Was mitigation involved?" and the adjuster answers "No", he has answered, and the negative branch is the correct value — set it, and cite his own answer turn as the source_span, never the agent\'s question. Silence is what does not count: a section nobody raised, or a question he talked past without answering, returns the field empty instead.',
    ...
    "The claim context identifies which claim this call was matched to — treat it as a hypothesis about which claim this is, not a settled fact. Use it to interpret references (\"the insured\", \"the property\") and to correct the spelling of a proper noun the transcript clearly refers to but a transcription error garbled — e.g. a contact's name or a carrier that is a near-miss for one in the claim context, but only where the adjuster's own words already corroborate the claim identity (he named the same insured, address, or claim number the claim context does). When the claim context includes an exact-match insured name and that corroboration holds, treat that spelling as authoritative over anything the call's voice-to-text produced — the insured's own name should never ride on transcription accuracy. Never use it as a source for a value the transcript never actually mentions. The ONLY comparison that can set claim_identity_mismatch is the adjuster's own words against the claim context. If the adjuster names a different insured, address, or claim number than the claim context names, the claim context loses its authority entirely: set claim_identity_mismatch.mismatched to true with a one-sentence reason naming the claim context's identity and the adjuster's identity explicitly, and also add an unplaced_notes entry saying the same, instead of silently overriding the transcript or trusting the claim context's spelling. An identity an AGENT proposed is never evidence of anything and is never one side of this comparison: an agent that opened with the wrong insured or address, and an adjuster who then corrected it, is this pipeline working correctly, not a mismatch. Compare the adjuster to the claim context and ignore the agent's proposal entirely — if the adjuster's identity matches the claim context, mismatched is false no matter what any agent proposed earlier in the call.",
  ]
...
var FIELD_GUIDANCE = {
  ...
  dwelling_stories:
    'The number of stories as the adjuster actually said it — e.g. "1 story", "2 story", "3 story" are the common cases, but use whatever he says (e.g. "a story and a half", "split level") rather than forcing it into one of those three. This value sits directly next to dwelling_type in "The dwelling is a [stories], [type] structure.", so a bare digit reads as broken: transcript "it\'s a one story" is "1 story", not "1" — "The dwelling is a 1, single-family home structure." is the defect this causes.',
  dwelling_type:
    'Completes "The dwelling is a [stories], ___ structure." — a noun phrase naming the dwelling type, e.g. "single family". The listed common values cover the normal cases; use the adjuster\'s own words when the transcript describes something else (e.g. "a townhome"). Write "single family", not "single-family home" — the sentence already supplies "structure", so a value that also carries "home structure" or "dwelling" duplicates it.',
  foundation_type:
  ...
  roof_covering_type:
    'Completes "The shingles on the roof are ___ that are approximately [age] years old." — a plural noun phrase naming the shingle product, e.g. "30 year laminate shingles" (the template supplies no article before this value, since it is meant to be plural). Use the adjuster\'s own words when the transcript names a product not in the common-values list rather than forcing the closest one. Include the head noun ("shingles") and use digits for the product\'s year rating: transcript "it\'s a thirty year laminate" is "30 year laminate shingles", not "thirty-year laminate" or "a thirty-year laminate" — the missing head noun and the spelled-out number both go into the same rendered sentence as roof_age_years\' digits ("approximately 23 years old"), and "a thirty-year laminate that are approximately 23 years old" is the ungrammatical, mixed-register result of skipping this.',
  roof_condition:
  ...
  mitigation_status:
    'Whether a mitigation vendor was involved. "present" needs the adjuster to describe mitigation work that was actually performed. "none" is the right value when he answers the mitigation question negatively ("no", "nothing was done", "no vendor has been out") — it renders a canned "No mitigation services were performed on this loss." line, so a negative answer is a complete answer on its own and needs no mitigation_narrative behind it. Cite his answer as the source_span, not the question that prompted it. Leave the field empty only when mitigation was never raised at all, or when he never actually answered.',
  ...
  front_elevation_status:
    'Findings for the front elevation only. The adjuster narrates the exterior as one continuous description and may identify an elevation by compass direction ("the north side") rather than "front" — infer which cardinal elevation is meant from context (e.g. the dwelling\'s orientation as discussed elsewhere in the call) and sort the finding into the matching front/right/back/left field; if the mapping is genuinely ambiguous, leave it unextracted and route the finding to unplaced_notes instead of guessing. If the transcript explicitly states there was no damage on this elevation, say so plainly. This field is required — if this elevation is genuinely never mentioned at all, leave it unextracted rather than inventing coverage of an elevation nobody discussed; it will be flagged for Brandon to fill in by hand. Write one coherent paragraph, never a denial followed by a contradicting finding: transcript "on the front side there\'s no damage really, well actually the fascia, three or four linear feet of it, was blown off, wind damaged and missing, that\'s probably where the water got into the bathroom" is "We observed approximately 3 to 4 linear feet of fascia on the front elevation that was blown off, wind-damaged, and missing, which appears to be the point where water entered the bathroom." — not "We observed no storm-related damages... [and then] a damage finding" as two contradicting sentences. Use digits and state a count once: transcript "there\'s two large glass panels, about sixty six inches by sixty inches, and there\'s two of them that got damaged" is "We observed two glass panels, approximately 66 inches by 60 inches, damaged on the front elevation." — not "two large glass panels... and there are two of them" with both measurements spelled out.',
  ...
  interior_status:
    'Choose "affected" when the adjuster reports any interior damage at all. "not_affected" is the right value when he answers the interior question negatively ("no", "nothing inside", "the interior was fine") — it renders "The dwelling interior was not affected during this loss.", so a negative answer is a complete answer on its own and needs no interior_damage_narrative behind it. Cite his answer as the source_span, not the question that prompted it. Leave the field empty only when the interior was never raised at all, or when he never actually answered — silence is not a "not_affected" answer.',
  ...
  overhead_profit_narrative:
    'State a determination (not included / included / not yet, but likely) plus a claim-specific reason (number of trades involved, scope of work, caliber of home). If coverage_determination is "excluded", use "Due to the coverage issue, overhead and profit do not appear to be applicable to this loss." instead of reasoning about trades or scope. If coverage_determination is "unknown", use "Overhead and profit will be determined once the coverage question on this loss is resolved." for the same reason. A determination alone is not a complete answer without its reason: "Overhead and profit do not apply." and "No overhead and profit considerations." are both incomplete. Write "Overhead and profit are not included on this loss given the single-trade scope of the roofing repair." or "Overhead and profit are included due to the multiple trades involved — roofing, drywall, and paint — and the coordination the scope requires." instead. That reason requirement applies to a determination the adjuster actually reasoned about. A bare negative answer to the overhead and profit question ("no", "no overhead and profit on this one") is itself his determination and is complete as it stands: write "Overhead and profit are not included on this loss." and cite his answer as the source_span, rather than inventing a trade count or a scope justification he never gave.',
  subrogation_reason:
    'Normally just the reason clause completing "There are no subrogation possibilities as the damages are ___." (e.g. "weather related", "related to a 10 year old plumbing supply line that was not recently repaired"). Start mid-sentence: no leading capital, no trailing period. If the transcript describes an unusual subrogation argument that does not fit this clause shape (e.g. a warranty-based argument), do not rewrite the whole sentence yourself — leave this field unextracted so Brandon writes it by hand. A clause is a noun phrase, not a sentence with its own verb: transcript "no subrogation concerns were reported" drops a finite verb ("were reported") into the slot and destroys the template sentence. A negative answer to the subrogation question does not belong in this slot at all — the template sentence already asserts there are no subrogation possibilities, and what the slot carries is the reason, which is the cause of loss. Echo the cause you extracted for origin_narrative, worded for this sentence ("weather related" for a storm, "related to a burst plumbing line due to freezing" for a freeze), and cite the cause-of-loss passage as the source_span rather than the subrogation exchange. Leave the field unextracted only when the adjuster never gave a cause anywhere in the call.',
  coinsurance_status:
