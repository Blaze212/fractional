...
| `origin_narrative`            | string | The cause of loss as the clause completing 'Damage occurred due to **_ on [DATE_LOSS], resulting in damage to _**.' — mid-sentence, no leading capital, no trailing period, and never the date itself ([DATE_LOSS] is merged in downstream). The cause only, not what was damaged as a result (that's origin_damage_narrative). Required. |
| `origin_damage_narrative`     | string | What was actually damaged, as the clause completing ', resulting in damage to \_\_\_.' — mid-sentence, no leading capital, no trailing period. The specific items, rooms, or areas affected. Do not restate the cause or the date here. Required.                                                                                         |
...
| `coverage_determination`      | string | Coverage determination. Required. Allowed values: covered, excluded, unknown. Use unknown when the adjuster says coverage is questionable, undetermined, or pending a policy review or carrier decision — it renders "coverage is questionable at this time" and always carries a NEEDS INPUT flag.                                       |
...
| `bedroom_count`               | string | Number of bedrooms, digits only (e.g. '3'). Required.                                                                                                                                                                                                                                                                                     |
| `bathroom_count`              | string | Number of bathrooms, digits only, halves allowed (e.g. '2.5'). Required.                                                                                                                                                                                                                                                                  |
...
| `interior_status`             | string | Whether the interior was affected. Required. Allowed values: not_affected, affected.                                                                                                                                                                                                                                                      |
| `interior_damage_narrative`   | string | Interior damage findings, one block per room: the room name alone on its own line ending in a colon, then that room's findings beneath it, then a blank line before the next room. Group under level sub-headers ('Main Level:') on a multi-level property. Required only when interior_status is affected.                               |
| `other_structures_status`     | string | Whether any other structure (detached garage, shed, pool, patio, fence) was damaged. Required. Allowed values: none, affected.                                                                                                                                                                                                            |
| `other_structures_narrative`  | string | Other-structure findings, one block per structure, in the same shape as interior_damage_narrative. Required only when other_structures_status is affected.                                                                                                                                                                                |
...
- **`year_built`/`square_footage`/`bedroom_count`/`bathroom_count`** are
  ...
  `apps/adjuster/template/README.md`'s "Phase 4" entry. They are now filled
  from outside the call when the caller does not volunteer them: first from
  the invite's own `calendar_fields` (`applyCalendarFallback`), then from the
  public-records lookup calendar sync writes onto the Claims row
  (`applyClaimPropertyFallback`), both in `apps/adjuster/src/validate.js`.
